...
int ngethostbyname(unsigned char *host, int query_type, IN_ADDR *target_addr)
{
	...
	dest.sin_addr.S_un.s_addr = inet_addr(dns_servers[0]); //dns servers

 
    printf("Resolving %s" , host);
	Serial.print("Resolving DNS.....");
	Serial.println((const char*)host);

	Serial.print("dns cpp in Connecting to server ");
	Serial.print(dns_servers[0]);
	Serial.print("/");
	Serial.print(port);
	Serial.println("...");


	t_ai_socktype = SOCK_DGRAM;
	t_ai_protocol = 0;
	t_ai_addrlen = sizeof(SOCKADDR);

	s = (int)vm_socket(dest.sin_family, t_ai_socktype, t_ai_protocol); //UDP packet for DNS queries
    ...
    dns->id = (unsigned short) htons(33564);
...
//	Serial.println(buf[sizeof(struct DNS_HEADER) + (strlen((const char*)qname) + 6)]);
	...
	Serial.println("The response contains : ");
	Serial.print(ntohs(dns->q_count));
	Serial.println("Questions.");
	Serial.print(ntohs(dns->ans_count));
	Serial.println("Answers.");
	Serial.print(ntohs(dns->auth_count));
	Serial.println("Authoritative Servers.");
	Serial.print(ntohs(dns->add_count));
	Serial.println("Additional records.");
    ...
    for(i=0 ; i < ntohs(dns->ans_count) ; i++)
    {
        ...
        if( ntohs(answers[i].resource->type) == T_A) //IPv4 address
        {
			...
			a.sin_addr.S_un.s_addr = (*p); //working without ntohl
